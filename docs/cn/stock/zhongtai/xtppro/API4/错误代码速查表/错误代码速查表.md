...
[ _下载XTP柜台错误代码速查表_](https://xtp.zts.com.cn/dl/sdk/XTPProErrorCode.xlsx) [ _上交所拒单原因错误代码查找链接_](https://view.officeapps.live.com/op/view.aspx?src=https%3A%2F%2Fwww.sse.com.cn%2Fservices%2Ftradingtech%2Fdata%2Fc%2F10832587%2Ffiles%2Ff66a50decd9941a995ace780e862cd29.xlsx&wdOrigin=BROWSELINK) [_深交所拒单原因错误代码查找链接_](https://www.szse.cn/marketServices/technicalservice/interface/P020240809670217642854.pdf)  
...
11011125| Invalid times_sh config in checking rule 25.| rule25上交所直接拒单的单量门限值无效.  
11011126| Invalid mid_brk_times_shsz config in checking rule 25.| rule25沪深交易所达到中间熔断临界单量门限值无效。  
11011127| Invalid brk_time_shsz config in checking rule 25.| rule25沪深交易所达到中间熔断临界单量时的熔断时间(单位秒)无效。  
11011128| Invalid enable_sh config in checking rule 27.| rule27上交所启用标识无效。  
11011129| Invalid times_sh config in checking rule 27.| rule27上交所收到指定时间下单次数门限值无效。  
11011130| Invalid in_time_sh config in checking rule 27.| rule27上交所指定时间内(单位秒)无效。  
11011131| Invalid brk_1st_times_shsz config in checking rule 27.| rule27沪深交易所第一阶段熔断豁免下单次数门限值无效。  
11011132| Invalid brk_2nd_time_shsz config in checking rule 27.| rule27沪深交易所第二阶段熔断时间(单位秒)无效。  
11011133| Invalid brk_2nd_times_shsz config in checking rule 27.| rule27沪深交易所第二阶段熔断下单次数门限值无效。  
11011134| Invalid brk_3rd_time_shsz config in checking rule 27.| rule27沪深交易所第三阶段熔断时间(单位秒)无效。  
11011135| Invalid brk_3rd_times_shsz config in checking rule 27.| rule27沪深交易所第三阶段熔断下单次数门限值无效。  
...
11011270| Failed to check risk rule 25 sz.| 风控指标rule25 sz 检查失败。  
11011314| Enter rule 14 breaker.| 进入风控指标rule14熔断。  
11011325| Enter rule 25 breaker sh.| 进入风控指标rule25 sh熔断。  
11011326| Enter rule 25 breaker sz.| 进入风控指标rule25 sz熔断。  
11011327| Enter rule 27 breaker.| 进入风控指标rule27熔断。  
11011328| Enter rule 27 breaker 1st sz.| 进入风控指标rule27 1st sz 熔断。  
11011329| Enter rule 27 breaker 2nd sh.| 进入风控指标rule27 2nd sh 熔断。  
11011330| Enter rule 27 breaker 2nd sz.| 进入风控指标rule27 2nd sz 熔断。  
11011331| Enter rule 27 breaker 3rd sh.| 进入风控指标rule27 3rd sh 熔断。  
11011332| Enter rule 27 breaker 3rd sz.| 进入风控指标rule27 3rd sz 熔断。  
...
11100000| exchange error code（深交所五位错误码）| 在英文错误消息中，可能出现20009的错误码，为深交所返回；表示委托价格超过了涨跌幅限制，具体可以参考深交所文档。<https://www.szse.cn/marketServices/technicalservice/interface/> 找最新版的《深圳证券交易所委托申报拒绝原因代码表》文档  
11110000| exchange error code（上交所错误码）| 在英文错误消息中，可能出现203这种类似的错误码，为上交所返回；具体可以参考上交所文档。<https://www.sse.com.cn/services/tradingtech/data/> 找最新版的《上海证券交易所报盘软件错误代码表》文档  
11114000| exchange error code（上交所流式报盘错误码）| 交易所拒单，为上交所返回；具体可以参考上交所文档，链接里<http://www.sse.com.cn/services/tradingtech/data/> 找交易网关Binary接口规格说明书里的错误代码表  
