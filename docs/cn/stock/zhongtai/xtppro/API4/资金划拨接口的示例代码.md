...
注意：上述代码的示例是针对XTP Pro API为1.1.0及其以上版本的用户可以参考的示例，对于XTP Pro API为1.0.15及其以下版本的，当用户Login成功后，资金划拨服务连接成功是收不到OnServerStatusNotification()的回调通知，只有连接失败才有该回调函数通知，所以不适用本文的示例代码，但如果用户Login成功返回后，就立即发起资金划拨操作，有可能会失败，建议更新API为1.1.0及其以上版本，参照本文的资金划拨示例。
