...
当用户与行情服务器之间采用的是TCP连接时，为了后续收到行情数据，需要重写**OnDisconnected()** 回调函数。 回调函数中的总体思路是循环调用**Login()** ，直至重连成功后，重新订阅快照行情。  
示例代码如下：
...
如果用户在断连后没有查询行情数据的需求，也没有行情回补以及动态订阅和退订的需求，此时可以不用重连。

### 3.2. 后续有其它接口调用的需求 ​

如果用户在断连后有查询行情数据的需求或者有行情回补需求，或者需要动态订阅新的合约行情和退订行情需求，那么在收到断线的回调通知OnDisconnected()里进行重连，重连成功后再进行其它请求操作，重连的示例代码如下。  
示例代码如下：
...
通常发生断线的时候，可能是网路状态不好导致，此时可能会有行情丢包发生。  
如果发现有行情丢包情形发生，可以通过调用请求回补行情接口，将丢失的数据回补回去。  
注意：XTP Pro行情回补与XTP行情回补不同，当与行情服务器发生断连时，要先重连行情服务器，再使用行情回补功能。  
具体回补行情的用法可以参阅《关于L2行情数据回补功能的使用说明》文档。
