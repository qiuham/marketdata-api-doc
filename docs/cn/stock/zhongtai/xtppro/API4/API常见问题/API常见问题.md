...
**2.1.51. 问：XTP-Pro平台行情支持过夜么？可以每天早上调用Login()，晚上自动Logout()吗？**

> 答：XTP-Pro服务器和API都不支持过夜，只支持当日交易时间段。策略程序必须每日重启，需要用户手动销毁QuoteApi退出程序，T+1日再重新开启程序，调用CreateQuoteApi()重新创建Api，再Login()。
...
> 答：QuoteApi不会自动帮用户重连，TCP连接断开后，如果需要重新建立连接，可以在收到断线通知后，选择不调用Release()销毁Api，不登出Logout()，在OnDisconnected()里直接登录Login()，如果登录不成功，也不要调用Logout()，继续while()循环登录，直到登录成功为止。  
>  注意：  
>  （1）实盘环境，如果程序是单订阅方式，还会动态再订阅个别合约，或者有查询（静态数据、最新价、最新快照）和回补需求，则需要重新Login()建立TCP连接。  
>  （2）测试环境，收到OnDisconnected()断线通知后，需要重新Login()建立TCP连接。

**2.2.4. 问：收到OnDisconnected()断线通知，在该回调函数里重新登录行情服务器成功后，需要重新订阅么？**

> 答：实盘环境下，收到OnDisconnected()回调通知时，只是表明TCP连接的断连，不会影响继续接收UDP组播行情，可根据实际情况决定是否需要建立TCP的重连。  
>  收到断线通知，可以有2个选择：  
>  （1）如果没有动态单订阅、查询（静态数据、最新价、最新快照）和回补需求，不用管断线，不需要在OnDisconnected()做任何处理；  
>  （2）如果有动态单订阅、查询（静态数据、最新价、最新快照）和回补需求，则需要在OnDisconnected()里断线重连，登录成功后，不需要重新订阅；  
>  在这2种情况下，用户都能持续收到UDP组播行情。但如果在收到断线通知后，调用Logout()或者Release()，会清掉加入的UDP组播，那么就不会收到行情通知了。  
>  注意：  
>  （1）在OnDisconnected()里断线重连，不要调用Logout()和Release()，直接调用Login()，如果登录不成功，也不要调用Logout()，而是sleep() 5秒左右（建议不小于3s）再登录，直到登录成功为止。  
>  （2）测试环境是TCP连接，断线后需要重新登录行情服务器，并重新订阅行情。
...
**3.1.36. 问：XTP-Pro平台交易支持过夜么？可以每天早上调用Login()，晚上自动Logout()吗？**

> 答：XTP-Pro服务器和API都不支持过夜，只支持当日交易时间段。策略程序必须每日重启，需要用户手动销毁TraderApi退出程序，T+1日再重新开启程序，调用CreateTraderApi() 重新创建Api，再Login()。
...
>   12. 是否下单数量过多，触发了风控。
>   13. 是否撤单过多，触发了风控。
>   14. 是否在允许报单时间内。 如果以上都不是，请看一下是否是模拟撮合环境，如果是模拟撮合环境，会有一定几率发生模拟交易所拒单。此时交易所拒单的error_code是11110000，或者11100000，error_msg通常为217或者10000或者29999。  
...
> 答：XTP-Pro中，只会返回当前节点对应市场的持仓，如果登录的上海节点，只会返回沪市的持仓，如果登录的深圳节点，只会返回深市的持仓。
...
> 答：如果没有开通一账户两中心，沪深的资金是合在一起的；如果开通了一账户两中心，沪深的资金是分开的，登录上海节点时，只会返回沪市的资金，登录深圳节点时，只会返回深市的资金。
...
**3.1.105. 问：XTP Pro柜台有Api接口查询金证主柜台的可用资金、可取资金吗？**

> 答：可调用 QueryOtherServerFund()接口查询，查询类型 XTPFundQueryReq.query_type 包括：金证主柜台可转资金、账户余额、可取资金。  
>  注意：  
...
>  (3) XTP_FUND_QUERY_JZ 为金证主柜台可转资金，即可用资金。  
>  (4) XTP_FUND_QUERY_JZ_WITHDRAW 为金证主柜台可取资金。仅现货账户在普通交易下，可取资金是准确的。如果涉及国债逆回购、ETF申赎业务，可取资金就没有参考意义了。  
>  (5) XTP_FUND_QUERY_JZ_BALANCE 为金证主柜台账户余额。若盘前金证主柜台账户余额、可转资金、可取资金均为10000元，在顺序操作以下场景后，资金变化如下：
> 
> 序号| 操作场景| 金证主柜台账户余额| 金证主柜台可转资金| 金证主柜台可取资金  
> ---|---|---|---|---  
> 1| 盘前 金证主柜台初始化| 10000| 10000| 10000  
> 2| 盘前 金证主柜台账户资金全部划转至XTP Pro柜台| 10000| 0| 0  
> 3| 盘中 XTP Pro柜台账户资金划转5000至金证主柜台| 10000| 5000| 5000  
> 4| 盘中 金证主柜台账户资金银证转出5000| 5000| 0| 0  
> 5| 盘中 XTP Pro柜台账户资金划转10000至金证主柜台| 5000| 10000| 5000  
> 6| 盘中 金证主柜台账户资金银证转入10000| 15000| 20000| 15000  
...
> 答：可调用QueryIPOInfoList()获取申购代码，参见返回的XTPQueryIPOTickerRsp.ticker。
...
> 答：今日可申购新股信息，可调用QueryIPOInfoList()查询最大允许申购数量，再调用QueryIPOQuotaInfo()查询可申购额度，那么所能申购的最大数量为：min（最大允许申购数量，新股可申购额度）。

**3.1.117. 问：今天查询新股申购额度QueryIPOQuotaInfo()，在信用账户和普通账户上都有申购额度，请问两个申购委托是独立的吗？**

> 答：不是独立的，两个账户是共享额度的。同一个一码通下的股东账户，若使用多个证券账户参与同一只新股申购，或同一证券账户对同一只新股多笔申购，交易所是按第一笔申购为有效申购，其余申购均为无效申购。  
>  如果重复申购报单，深交所会报错：20039 重复申购，上交所不会拒单，实际清算只以第一笔申购为有效申购。

**3.1.118. 问：XTP中下单的新股申购，是在XTP柜台缴款，还是在普通柜台缴款？**

> 答：XTP中报单新股申购不会预扣认购资金，待新股中签后，在XTP柜台和金证柜台都可以缴款。在缴款期间，需在XTP或齐富通留有足够的新股认购资金，如果资金不足会导致申购失败。

**3.1.119. 问：XTP中有API接口查询新股中签情况吗？**

> 答：API没有接口查询新股中签，而且XTP中查不到中签情况，可以通过齐富通或融易汇在金证主柜台查询。

**3.1.120. 问：如果某只股票发生配股，需要配股的话，如何操作？**

> 答：先查询可配股合约的持仓，可根据OnQueryPosition()返回的position_security_type过滤出配售类型的持仓，包含配股、配债等。其中：  
>  （1）yesterday_position 表示总可配股数量；  
>  （2）sellable_qty 表示剩余可配股数量；  
>  （3）total_qty 表示已配股数量；  
>  假如股票A 可配股票B 1000股，则持仓中股票B初始数量为：yesterday_position = 1000，sellable_qty=1000，total_qty=0；  
>  报单操作配股200股后，则查询持仓中股票B数量为：yesterday_position = 1000，sellable_qty=800，total_qty=200。

**3.1.121. 问：查询可转债申购是哪个接口？申购单位是手还是张？**

> 答：QueryBondIPOInfoList()接口查询今日可转债申购列表，申购最小单位为10张（1000元），申购数量应当为10张或10张的整数倍。  
>  注意：在OnQueryBondIPOInfoList()回调中，证券类型 参见 ticker_type 字段，债券是 XTP_TICKER_TYPE_BOND。

**3.1.122. 问：可转债转股后的股票证券代码、转股价格，是调用哪个接口查询？**
...
> 答：在服务器与客户端断线的时候会被回调，但是用户主动调用Logout()登出时不会触发OnDisconnected()。

**3.2.3. 问：TraderApi断线后会自动重连么？**

> 答：TraderApi不会自动帮用户重连。用户可以在收到断线通知OnDisconnected()后，在该函数中再次调用Login()，但不要调用Logout()和Release()，登录失败也不要调用Logout()，直到登录成功为止，并在登录成功后更新session_id即可。交易服务器会在用户重新登录后，从断点消息处续传。

**3.2.4. 问：交易程序在断线后，如何系统恢复？**

> 答：收到OnDisconnect()断线通知后，可以有2种方式来恢复：  
>  （1）在不重启程序的情况下，在OnDisconnected()中不调用Logout()和Release()，直接调用Login()登录，默认就是XTP_TERT_RESUME方式，登录成功后只会收到断线后的消息续传；  
>  （2）在重启程序的情况下，通过XTP_TERT_RESTART方式登录，或者XTP_TERT_QUICK方式登录+查询（QueryOrders/QueryTrades）来获取今日所有的公共流（订单响应、成交回报）消息数据。
...
> 答：用户收到此回调，仅表明已经认证成功，此时还不能向服务器发送报单和查询等操作，需要等Login()返回才能进行后续下单操作，此处可提前记录并更新用户名与session_id的对应关系。
...
> 答：qty_upper_limit是指这只股票的最大允许申购数量，还需查询可申购额度，参见XTPQueryIPOQuotaRsp，请使用min（最大允许申购数量，可申购额度）进行新股申购。
...
**5.2.2. 问：公网测试环境，行情数据更新频率是怎样的？收盘后有回放行情可以测试吗**

> 答：公网测试环境的仿真行情数据，盘中推送频率跟实盘一致。收盘后也有循环回放行情可以测试，推送频率比盘中要快。  
>  (1) 公网测试环境，交易日在15:30、19:00启动回放服务，回放当天的行情数据。周末及节假日在8:00、12:00、15:30、19:00启动，回放上一个交易日的行情数据。  
>  (2) 行情回放持续时间不固定，根据当天实际的行情量来确定，沪市行情的回放速率是0.5M/S，深市是5000笔/S，行情数据时间参见data_time。
...
> 答：公网测试环境除了网络延迟比较大之外，模拟撮合的速度也有一定的延迟，以上交所模拟撮合为例，是1秒一个周期进行轮询，如果下单的时候正好进入下一个周期，那么最晚需要1秒才会收到订单响应。
