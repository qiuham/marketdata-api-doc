---
exchange: bybit
source_url: https://bybit-exchange.github.io/docs/v5/sbe/sbe-public-trade
api_type: REST
updated_at: 2026-01-16T09:41:03.977083
---
...
side (5)| SideType| “Side of taker” (commonly Buy/Sell)  
isBlockTrade (6)| uint8| IsBlockTrade(0 = not blockTrade, 1 = blockTrade)  
isRPI (7)| uint8| IsRPI (0 = not RPI, 1 = RPI)  
execId (8)| int64| Trade ID  
...
tradeItems| 40| group(`groupSize16Encoding`)| -| 重複的成交項目（Repeating trade items）  
...
seq（4）| int64| 撮合序列 ID  
side（5）| SideType| 「吃單方方向」（常見為 Buy/Sell）  
isBlockTrade（6）| uint8| 是否為大宗交易（0 = 非 blockTrade，1 = blockTrade）  
isRPI（7）| uint8| 是否為 RPI（0 = 非 RPI，1 = RPI）  
