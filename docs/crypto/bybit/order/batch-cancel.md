...
api_type: Trading
updated_at: 2026-09-23 18:45:53.082953
---
...
rpiTakerAccess| false| boolean| `true`: Your order will be eligible to match against RPI quotes, `false`: not eligible. Supported order combinations: (1) `orderType=Market`; (2) `orderType=Limit` with `timeInForce=IOC` or `FOK`. Please read the [RPI taker feature](https://announcements.bybit.com/en/article/rpi-liquidity-now-available-to-api-taker-orders-bltb943887bfa4c4d17/)  
...
rpiTakerAccess| false| boolean| `true`: 您的訂單可以和RPI報價撮合, `false`: 不能和RPI報價撮合. 支持的訂單組合：(1) `orderType=Market`；(2) `orderType=Limit` 且 `timeInForce=IOC` 或 `FOK`。請先仔細閱讀[RPI Taker](https://announcements.bybit.com/en/article/rpi-liquidity-now-available-to-api-taker-orders-bltb943887bfa4c4d17/)的特性  
