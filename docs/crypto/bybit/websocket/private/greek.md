...
api_type: WebSocket
updated_at: 2026-09-24 18:49:45.887447
---
...
> netDeltaRatio| string| Net delta ratio. 計算不包含期權 delta 部分. 計算方式: `coinAccountSize = abs(longSize + shortSize)`, `coinBaseSize = max(longSize, abs(shortSize))`, `netDeltaRatio = coinAccountSize / coinBaseSize`. 未開啟 delta neutral 模式時返回 `""`. 請調用 [POST /v5/account/set-delta-mode](/docs/zh-TW/v5/account/set-delta-mode) 開啟 delta neutral 模式.  
