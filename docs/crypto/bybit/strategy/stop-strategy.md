...
api_type: REST
updated_at: 2026-08-08 18:50:38.366679
---
...
Access via the Web UI
  ...
  * **TradFi → CFD** ：This section covers forex, commodities, and stock CFDs.
  * **TradFi → Futures** ：Commodities perpetuals, Stock perpetuals (e.g., TSLAUSDT), ETF perpetuals (e.g.,QQQUSDT)



![TradFi\(CFD &amp; Futures\) Web UI Entry](/docs/assets/images/tradfi-webui-entry-39eb4cc8f8a74047a738f096792d0d6d.png)

### Bybit Platform (Native)

  * **Trade → Spot** : xStock tokens (e.g., TSLAX, AAPLX, GOOGLX)
  * **Trade → Options** : Options contracts settled in USDT, including tokenised gold options (XAUTUSDT-Options)
...
透過 Web UI 進入
  ...
  * **TradFi → CFD** ：此區塊涵蓋外匯、大宗商品及股票 CFD。
  * **TradFi → Futures** ：大宗商品永續合約、股票永續合約（如 TSLAUSDT）、ETF 永續合約 (e.g.,QQQUSDT)



![TradFi\(CFD &amp; Futures\) Web UI 入口](/docs/zh-TW/assets/images/tradfi-webui-entry-39eb4cc8f8a74047a738f096792d0d6d.png)

### Bybit 平台（原生）

透過主選單 **Trade** 進入，包含所有標準的 Bybit 產品類型：

  * **Trade → Spot** ：xStock 代幣（如 TSLAX、AAPLX、GOOGLX）
  * **Trade → Options** ：以 USDT 結算的期權合約，包括代幣化黃金期權（XAUTUSDT-Options）
